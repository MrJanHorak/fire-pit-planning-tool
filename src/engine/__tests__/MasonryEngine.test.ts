...
describe('MasonryEngine', () => {
  ...
  it('puts propane vents at base and natural gas vents near top', () => {
    ...
    const propane = engine.calculateDesign({ ...baseInput, fuelType: 'propane' })
    const naturalGas = engine.calculateDesign({ ...baseInput, fuelType: 'natural-gas' })
    ...
    expect(naturalGas.ventSpec.targetCourseIndexes[0]).toBe(naturalGas.courses.length - 1)
  })
  ...
  it('calculates purchased units and logistics with waste factor', () => {
    ...
    expect(output.logistics.purchasedUnits).toBe(Math.ceil(output.totalUnits * 1.15))
    expect(output.logistics.estimatedBrickWeightLb).toBeGreaterThan(output.totalUnits * 4)
    ...
    expect(output.logistics.estimatedStoneWeightLb).toBeGreaterThan(output.foundation.stoneVolumeCubicFeet * 100)
  ...
  it('increases cap course count when capstone overhang increases', () => {
    const engine = new MasonryEngine()
    const compactCap = engine.calculateDesign({ ...baseInput, capstoneOverhangIn: 0.5 })
    const wideCap = engine.calculateDesign({ ...baseInput, capstoneOverhangIn: 2.5 })

    expect(wideCap.capstone.capOuterDiameterIn).toBeGreaterThan(compactCap.capstone.capOuterDiameterIn)
    expect(wideCap.capstone.capUnitsPerCourseRounded).toBeGreaterThan(compactCap.capstone.capUnitsPerCourseRounded)
  })
})

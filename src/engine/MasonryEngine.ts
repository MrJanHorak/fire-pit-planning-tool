...
export class MasonryEngine {
  ...
  public calculateDesign(input: MasonryInput, unit: MasonryUnit = this.defaultUnit): MasonryOutput {
    const oriented = this.resolveUnit(input, unit)
    const unitsPerCourseRaw = this.calculateCircularUnitCountRaw(input.innerDiameterIn, oriented.widthIn, oriented.lengthIn, input.mortarJointIn)
    const unitsPerCourseRounded = Math.max(1, Math.floor(unitsPerCourseRaw))

    const courseCount = Math.max(1, Math.ceil(input.wallHeightIn / (oriented.heightIn + input.mortarJointIn)))
    const courses = this.buildRunningBondCourses(courseCount, unitsPerCourseRounded, oriented.lengthIn, input.mortarJointIn)
    ...
    const foundation = this.calculateFoundation(input.innerDiameterIn + oriented.widthIn * 2)
    ...
    return {
      ...
      foundation,
      capstone,
      logistics: this.calculateLogistics(totalUnits, capstone.capUnitsPerCourseRounded, foundation),
      warnings,
    }
  }

  public calculateCircularUnitCountRaw(innerDiameterIn: number, wallWidthIn: number, unitLengthIn: number, jointIn: number): number {
    const outerDiameterIn = innerDiameterIn + 2 * wallWidthIn
    return (Math.PI * (outerDiameterIn - wallWidthIn)) / (unitLengthIn + jointIn)
  }
  ...
  private buildRunningBondCourses(courseCount: number, unitCount: number, unitLengthIn: number, jointIn: number): CoursePlan[] {
  ...
  private calculateCapstone(
    ...
    const capUnitsPerCourseRaw = (Math.PI * (capOuterDiameterIn - wallWidthIn)) / (unitLengthIn + jointIn)
  ...
  private calculateLogistics(totalUnits: number, capUnits: number, foundation: FoundationSpec): LogisticsSpec {
    const purchasedUnits = Math.ceil(totalUnits * (1 + BRICK_WASTE_FACTOR_PCT / 100))
    const purchasedCapUnits = Math.ceil(capUnits * (1 + CAP_WASTE_FACTOR_PCT / 100))
    const stoneWithWasteFt3 = foundation.stoneVolumeCubicFeet * (1 + STONE_WASTE_FACTOR_PCT / 100)
  ...
  private computeSafetyWarnings(proximityToStructuresFt: number): SafetyWarning[] {
    const warnings: SafetyWarning[] = []

    if (proximityToStructuresFt < 10) {
      warnings.push({
        code: 'clearance-too-low',
        message: 'Minimum horizontal clearance is 10 ft from combustible structures.',

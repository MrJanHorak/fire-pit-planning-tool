  ...
  return (
    <section className='card-rise rounded-2xl border border-amber-900/20 bg-amber-50/80 p-4 shadow-lg'>
      ...
      <p className='mb-3 text-sm text-amber-950/80'>
        Each row is one course. Alternating start offsets implement running
        bond. Vent target courses are marked in red.
      </p>

      <svg
        viewBox={`0 0 940 ${svgHeight}`}
        className='w-full rounded-lg border border-amber-900/20 bg-white'
      >
            ...
            <g key={course.courseIndex}>
              ...
              {Array.from({ length: course.unitCount }, (_, brickIdx) => (
                <rect
                  ...
                  rx={2}
                  fill={
                    ventCourse && brickIdx % 5 === 0 ? '#c13a1f' : '#b66a34'
                  }
